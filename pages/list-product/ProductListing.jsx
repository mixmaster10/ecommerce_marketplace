import React, {useState} from 'react';


const ProductListing = (props) => {

    const [showListView, setShowListView] = useState(false)


    //http://ecom-mysql.api.bluejestic.local/products
    //http://localhost:1337/products
    fetch(`http://ecom-mysql.api.bluejestic.local/products`)
        .then(resp => resp.json())
        .then(data => {
            console.log('returned from fetch products:',data)
            //setState({ stores: data })
        })
  


    return (
        <div>
            <div className="product-list  chat-box">
                <div className="product-list-header">
                    <div className="all-listing"><span>All Listings</span><a href="#" className="btn btn-rounded">50</a></div>
                    <div className="product-list-header-right">
                        <div className="list-view"><span>View:</span><a onClick={() => setShowListView(!showListView)}><i className="fa fa-bars"></i></a> <a onClick={() => setShowListView(!showListView)}><i className="fa fa-th"></i></a></div>
                        <div className="list-sort"><span>Sort:</span><a href="#" className="newly-listed">Newly Listed</a></div>
                    </div>
                </div>
                {showListView ? (
                <div className="product-list-items list">
                    <div className="row">
                        <div className="col-lg-12 mb-4">
                            <div className="product product-item">
                                <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/07.png" alt="" /></div>
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-info">
                                        <div className="product-action">
                                            <div className="product-price">$55.99</div>
                                            <div className="product-shipping"><i className="fa fa-truck"></i>Free Shipping</div>
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                        <p>Lauren Ralph Lauren Women's Quilted</p>
                                    </div>
                                    <div className="product-item-footer">
                                        <div className="footer-left">
                                            <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>422</span></a></div>
                                            <div className="product-item-like">199 likes</div>
                                            <div className="viewers">
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1557862921-37829c790f19?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1562003349-2483c92da7b3?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="data:image/jpeg;base64," /></a>
                                                <a href="#"><img className="img-fluid" src="https://static.billboard.com/files/2020/03/Joe-Diffie-2020-cr-Crystal-K-Martel-billboard-1548-1585338296-compressed.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://i.pinimg.com/originals/3c/10/db/3c10dbbcc4925031bb10d8cccf650c37.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1590086783191-a0694c7d1e6e?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=934&q=80" /></a>
                                            </div>
                                            <div className="trending">Trending <a href="#" className="trending-btn">+ 0.2%</a></div>
                                        </div>
                                        <div className="footer-right">
                                            <div className="product-shop">
                                                <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                            </div>
                                            <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                        </div>
                                    </div>
                                </div>

                            </div>
                        </div>


                        <div className="col-lg-12 mb-4">
                            <div className="product product-item">
                                <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/02.png" alt="" /></div>
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-info">
                                        <div className="product-action">
                                            <div className="product-price">$392.99</div>
                                            <div className="product-shipping"><i className="fa fa-truck"></i>Free Shipping</div>
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                        <p>Bose Ear-Phone Bluetooth & Wireless</p>
                                    </div>
                                    <div className="product-item-footer">
                                        <div className="footer-left">
                                            <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>324</span></a></div>
                                            <div className="product-item-like">1.2k likes</div>
                                            <div className="viewers">
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1557862921-37829c790f19?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1562003349-2483c92da7b3?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="data:image/jpeg;base64," /></a>
                                                <a href="#"><img className="img-fluid" src="https://static.billboard.com/files/2020/03/Joe-Diffie-2020-cr-Crystal-K-Martel-billboard-1548-1585338296-compressed.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://i.pinimg.com/originals/3c/10/db/3c10dbbcc4925031bb10d8cccf650c37.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1590086783191-a0694c7d1e6e?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=934&q=80" /></a>
                                            </div>
                                            <div className="trending">Trending <a href="#" className="trending-btn">+ 0.2%</a></div>
                                        </div>
                                        <div className="footer-right">
                                            <div className="product-shop">
                                                <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                            </div>
                                            <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>


                        <div className="col-lg-12 mb-4">
                            <div className="product product-item">
                                <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/01.png" alt="" /></div>
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-info">
                                        <div className="product-action">
                                            <div className="product-price">$455.99</div>
                                            <div className="product-price-del"><del>$45.00 OFF</del></div>

                                            <div className="product-shipping"><i className="fa fa-truck"></i>Free Shipping</div>
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                        <p>Edifier Powered Bookshelf Speaker</p>
                                    </div>
                                    <div className="product-item-footer">
                                        <div className="footer-left">
                                            <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>324</span></a></div>
                                            <div className="product-item-like">1.2k likes</div>
                                            <div className="viewers">
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1557862921-37829c790f19?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1562003349-2483c92da7b3?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="data:image/jpeg;base64," /></a>
                                                <a href="#"><img className="img-fluid" src="https://static.billboard.com/files/2020/03/Joe-Diffie-2020-cr-Crystal-K-Martel-billboard-1548-1585338296-compressed.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://i.pinimg.com/originals/3c/10/db/3c10dbbcc4925031bb10d8cccf650c37.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1590086783191-a0694c7d1e6e?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=934&q=80" /></a>
                                            </div>
                                            <div className="trending">Trending <a href="#" className="trending-btn">+ 0.2%</a></div>
                                        </div>
                                        <div className="footer-right">
                                            <div className="product-shop">
                                                <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                            </div>
                                            <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>


                        <div className="col-lg-12 mb-4">
                            <div className="product product-item">
                                <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/09.png" alt="" /></div>
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-info">
                                        <div className="product-action">
                                            <div className="product-price">$349.99</div>
                                            <div className="product-shipping"><i className="fa fa-truck"></i>Free Shipping</div>
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                        <p>Lauren Ralph Lauren Women's Quilted</p>
                                    </div>
                                    <div className="product-item-footer">
                                        <div className="footer-left">
                                            <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>714</span></a></div>
                                            <div className="product-item-like">1k likes</div>
                                            <div className="viewers">
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1557862921-37829c790f19?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1562003349-2483c92da7b3?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="data:image/jpeg;base64," /></a>
                                                <a href="#"><img className="img-fluid" src="https://static.billboard.com/files/2020/03/Joe-Diffie-2020-cr-Crystal-K-Martel-billboard-1548-1585338296-compressed.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://i.pinimg.com/originals/3c/10/db/3c10dbbcc4925031bb10d8cccf650c37.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1590086783191-a0694c7d1e6e?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=934&q=80" /></a>
                                            </div>
                                            <div className="trending">Trending <a href="#" className="trending-btn">+ 0.2%</a></div>
                                        </div>
                                        <div className="footer-right">
                                            <div className="product-shop">
                                                <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                            </div>
                                            <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>


                        <div className="col-lg-12 mb-4">
                            <div className="product product-item">
                                <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/05.png" alt="" /></div>
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-info">
                                        <div className="product-action">
                                            <div className="product-price">$309.99</div>
                                            <div className="product-shipping"><i className="fa fa-truck"></i>Free Shipping</div>
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                        <p>Apple iPhone 8 Plus - 64GB - Space Gray</p>
                                    </div>
                                    <div className="product-item-footer">
                                        <div className="footer-left">
                                            <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>6274</span></a></div>
                                            <div className="product-item-like">1k likes</div>
                                            <div className="viewers">
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1557862921-37829c790f19?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1351&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1562003349-2483c92da7b3?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=1350&q=80" /></a>
                                                <a href="#"><img className="img-fluid" src="data:image/jpeg;base64," /></a>
                                                <a href="#"><img className="img-fluid" src="https://static.billboard.com/files/2020/03/Joe-Diffie-2020-cr-Crystal-K-Martel-billboard-1548-1585338296-compressed.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://i.pinimg.com/originals/3c/10/db/3c10dbbcc4925031bb10d8cccf650c37.jpg" /></a>
                                                <a href="#"><img className="img-fluid" src="https://images.unsplash.com/photo-1590086783191-a0694c7d1e6e?ixid=MXwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHw%3D&ixlib=rb-1.2.1&auto=format&fit=crop&w=934&q=80" /></a>
                                            </div>
                                            <div className="trending">Trending <a href="#" className="trending-btn">+ 0.2%</a></div>
                                        </div>
                                        <div className="footer-right">
                                            <div className="product-shop">
                                                <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                            </div>
                                            <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
                ) : <div className="product-list grid chat-box">
                <div className="product-list-items">
                    <div className="row">
                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/02.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$392.99</span>
                                        <p>Bose Ear-Phone Bluetooth & Wireless</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>324</span></a></div>
                                        <div className="product-item-like">1.2k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/03.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$242.99</span>
                                        <p>iQOS 2.4 Plus Kit, Holder & Chargers</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>241</span></a></div>
                                        <div className="product-item-like">2.7k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/04.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$515.99</span>
                                        <p>Samsung TU8000 - 55" Crystal 4K UHD HDR</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>521</span></a></div>
                                        <div className="product-item-like">7k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>


                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/06.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$29.99</span>
                                        <p>Bose Ear-Phone Bluetooth & Wireless</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>127</span></a></div>
                                        <div className="product-item-like">302 likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>


                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/01.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$455.99</span>
                                        <p>Edifier Powered Bookshelf Speaker</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>234</span></a></div>
                                        <div className="product-item-like">1k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/05.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$309.99</span>
                                        <p>Apple iPhone 8 Plus - 64GB - Space Gray</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>627</span></a></div>
                                        <div className="product-item-like">1k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/07.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$55.99</span>
                                        <p>Lauren Ralph Lauren Women's Quilted</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>422</span></a></div>
                                        <div className="product-item-like">199 likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/08.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$99.99</span>
                                        <p>LAUREN RALPH LAUREN Wool ...</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>531</span></a></div>
                                        <div className="product-item-like">1.3k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>

                        <div className="col-lg-4 col-md-6 mb-4">
                            <div className="product product-item">
                                <div className="product-content">
                                    <div className="post-item-menu">
                                        <a href="#"><i className="fa fa-ellipsis-v"></i></a>
                                    </div>
                                    <div className="product-item-bg"><img className="img-fluid" src="/static/img/product/09.png" alt="" /></div>
                                    <div className="product-info">
                                        <span className="product-price">$349.99</span>
                                        <p>RALPH LAUREN Women's Burnished</p>
                                        <div className="product-action">
                                            <a href="#" className="add-btn">Add</a>
                                            <a href="#" className="buynow-btn">Buy now</a>
                                        </div>
                                    </div>
                                </div>
                                <div className="product-item-footer">
                                    <div className="footer-left">
                                        <div className="product-item-comment"><a href="#" className="product-comments"><i className="fa fa-comments"></i><span>714</span></a></div>
                                        <div className="product-item-like">1k likes</div>
                                    </div>
                                    <div className="footer-right">
                                        <div className="product-shop">
                                            <a href="#"><span className="shop-bg"><img className="img-fluid" src="/static/img/tavrovska-logo.jpg" alt="" /></span><span>Tavrovska</span></a>
                                        </div>
                                        <div className="product-item-save"><a href="#"><i className="fa fa-bookmark"></i></a></div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>}
            </div>

        </div>
    );
};

export default ProductListing;
